...
// ── Layout shell + design tokens (used by Tasks 5–7) ──────────────────────

const _MESSAGES: Record<string, unknown> = { de, en, es, fr, it, pt };

const SUPPORTED_LOCALES = ['de', 'en', 'es', 'fr', 'it', 'pt'] as const;
type _SupportedLocale = (typeof SUPPORTED_LOCALES)[number];

function resolveLocale(locale: string): _SupportedLocale {
  return (SUPPORTED_LOCALES as readonly string[]).includes(locale)
    ? (locale as _SupportedLocale)
    : 'en';
}
...
function makeT(locale: string, namespace: string) {
  ...
  return createTranslator({ locale: lc, messages: _MESSAGES[lc] as Record<string, any>, namespace });
}
